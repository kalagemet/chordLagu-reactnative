...
export const setLoginStatus = async (value, onSaved) => {
  ...
  await AsyncStorage.setItem("@login", string).then(() => {
    onSaved();
  });
};
...
export const setUserInfo = async (value, onSaved) => {
  ...
  await AsyncStorage.setItem("@userInfo", string).then(() => {
    onSaved();
  });
};
...
export const saveSong = async (value, onSaved) => {
  setSavedList(value);
  let string = JSON.stringify(value);
  await AsyncStorage.setItem("@" + value.id, string).then(() => {
    onSaved();
  });
};
...
export const deleteSaved = async (id, onDeleted) => {
  ...
  await AsyncStorage.setItem("@saved", JSON.stringify(saved)).then(onDeleted());
};
...
export const setLocalAppVersion = async (value, onSaved) => {
  let string = value.toString();
  await AsyncStorage.setItem("@version", string).then(() => {
    onSaved();
  });
};
